...
[[ -f "${ENV_FILE}" ]] || { echo "Missing app/.env; run ./bootstrap_prithi_studio.sh and configure it." >&2; exit 1; }
...
if curl -fsS --max-time 3 -H "Authorization: Bearer ${PRITHI_WEB_ACCESS_TOKEN}" "http://127.0.0.1:${PORT}/api/health" >/dev/null 2>&1; then
  ...
  if [[ -f "${WEB_PID_FILE}" ]] && kill -0 "$(<"${WEB_PID_FILE}")" 2>/dev/null; then
    echo "A recorded Prithi web process exists but is not healthy; inspect ${WEB_LOG}." >&2; exit 1
  fi
  rm -f "${WEB_PID_FILE}"
...
if [[ -f "${CRED}" ]]; then echo "TTS credentials configured: yes"; else echo "TTS credentials configured: no"; fi
