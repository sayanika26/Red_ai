...
def analyze_context(text: str, selected_language: str | None = None) -> TurnContext:
    ...
    joke_request = any(x in low for x in ("tell me a joke", "say a joke", "একটা জোক", "একটা কৌতুক", "কৌতুক বল", "জোক বল", "चुटकुला", "जोक सुनाओ"))
    playful = joke_request or any(x in low for x in ("joke", "tease", "হাসাও", "মজা", "ঠাট্টা", "मज़ाक", "चिढ़ाओ"))
    ...
    intent = "deescalate" if stop else "set_boundary" if negative else "seek_support" if sad else "tell_joke" if joke_request else "play" if playful else "connect" if affection or intimate else "ask" if question else "share"
